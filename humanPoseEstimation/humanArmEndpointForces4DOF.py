...
js_frame = ReferenceFrame('Js')
j1_frame = ReferenceFrame('J1')
j2_frame = ReferenceFrame('J2')

#declare dynamic symbols for the three joints
theta0, thetas, theta1, theta2 = dynamicsymbols('theta0, thetas, theta1, theta2')

#orient frames
j0_frame.orient(inertial_frame,'Axis',(theta0, inertial_frame.y))
js_frame.orient(j0_frame, 'Axis',(thetas, j0_frame.x))
j1_frame.orient(js_frame,'Axis',(theta1, js_frame.z))
...
joints = Point('js')
js_length = symbols('l_js')
joints.set_pos(joint0, j0_length*j0_frame.y)
joint1 = Point('j1')
joint1.set_pos(joints, j0_length*j0_frame.y)
...
#COM locations
j0_com_length, js_com_length, j1_com_length, j2_com_length = symbols('d_j0, d_js, d_j1, d_j2')
j0_mass_center = Point('j0_o')
j0_mass_center.set_pos(joint0, j0_com_length * j0_frame.y)

js_mass_center = Point('js_o')
js_mass_center.set_pos(joints, js_com_length * js_frame.y)
...
omega0, omegas, omega1, omega2 = dynamicsymbols('omega0, omegas, omega1, omega2')
...
js_frame.set_ang_vel(j0_frame,omegas*j0_frame.x)
j1_frame.set_ang_vel(js_frame,omega1*js_frame.z)
...
joints.v2pt_theory(joint0, inertial_frame, j0_frame)
js_mass_center.v2pt_theory(joints, inertial_frame, j0_frame)
joint1.v2pt_theory(joints, inertial_frame, js_frame)
...
#Inertia---------------------------------------------------------------
j0_mass, js_mass, j1_mass, j2_mass = symbols('m_j0, m_js, m_j1, m_j2')
j0_inertia, js_inertia, j1_inertia, j2_inertia = symbols('I_j0, I_js, I_j1, I_j2')
...
js_inertia_dyadic = inertia(js_frame, 0.012, 0.01, 0.0114, ixy = 0.00007, iyz = -0.00002, izx = -0.0002) #DEBUG ----- temporary values
js_central_inertia = (js_inertia_dyadic, js_mass_center)
...
links = RigidBody('Link s', js_mass_center, js_frame, js_mass, js_central_inertia)
...
js_grav_force = (js_mass_center, 0*inertial_frame.y)
...
#joint torques
j0_torque, js_torque, j1_torque, j2_torque = dynamicsymbols('T_j0, T_js, T_j1, T_j2')
...
ls_torque = (js_frame, js_torque*j0_frame.y) #NEEDS DEBUG -----
...
#Equations of Motion----------------------------------------------------
coordinates = [theta0, thetas, theta1, theta2]
speeds = [omega0, omegas, omega1, omega2]
...
loads = [#j0_grav_force, 
		 ...
		 l1_torque,
		 l2_torque]
bodies = [link0, links, link1, link2]
...
constants = [j0_length,
			 ...
			 EE_force_y,
			 EE_force_z]
specified = [j0_torque, js_torque, j1_torque, j2_torque]
